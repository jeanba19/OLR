...
def OLR_MTS(T,D,B,K,A,Q,P,nu,mu,nuExt,muExt):
    ...
    while t <=T:
        ...
        #update price p_t before decision
        p_t = np.array([P[t]])
        mult_t = np.concatenate((p_t,q_k))
        
        #if price p_t is revealed after decision
        #mult_t = Mult_t[t-1]
        ...
        l=max(0,l+(p_t*x_prev[0] + np.dot(q_k,slots) - B))


        t=t+1
    
    return results, vect_fit, ext_results, ext_vect_fit
